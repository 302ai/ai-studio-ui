...
<script lang="ts">
	...
	import { Lightbulb, Image, Music, Play, Hammer, PenLine, Trash2, Star } from "@lucide/svelte";

	let { model, isLast = false, onEdit, onDelete, onToggleCollected }: Props = $props();
...
<div
	class={cn(
		"h-[50px] ring-primary outline-transparent hover:bg-muted/50",
	...
	<div class="flex h-full">
		...
		<div class="flex h-full min-w-[100px] flex-[1.2] items-center gap-2 outline-hidden">
			{#each Array.from(model.capabilities) as capability (capability)}
				...
				{#if capabilityInfo}
					<div
						class={cn("flex size-6 items-center justify-center rounded-sm", capabilityInfo.bgClass)}
